...
from dataset import CategoryChoiceEmbeddingDataset
...
if __name__ == "__main__":
    ...
    depth_dir = "Data/ProcessedData/subset_template_200_dmaps_fixed"
    embed_dir = "Embeddings/PRETRAINED/subset_template_200_fixed"
    save_dir = "TrainedModels/ALIGN/subset_200_direct_new_loss_fixed/"
    os.makedirs(save_dir, exist_ok=True)
    save_interval = 20

    
    all_categories = ['PillBottle', 'Books', 'Couch', 'Plant', 'Truck', 'Table', 'ChestOfDrawers', 'DrinkingUtensil', 'Bed', 'Fan', 'CeilingFan', 
                  'Showerhead', 'Vase', 'PersonStanding', 'Faucet', 'MediaStorage', 'PottedPlant', 'USBStick', 'Camera']
    label_to_idx = {label: idx for idx, label in enumerate(all_categories)}

    dataset = CategoryChoiceEmbeddingDataset(dataset_path, embed_dir, label_to_idx)
    ...
    optimizer = torch.optim.AdamW(align_model.parameters(), lr=1e-4)
